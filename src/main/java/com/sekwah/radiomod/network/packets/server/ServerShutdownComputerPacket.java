...
public class ServerShutdownComputerPacket implements IMessage {
    ...
    public ServerShutdownComputerPacket() { }

    public ServerShutdownComputerPacket(boolean sendToAll, BlockPos pos) {
        ...
        public IMessage onMessage(ServerShutdownComputerPacket message, MessageContext ctx) {
        	...
        	if(tileEntity != null) {
        		...
        		if(message.sendToAll) {
            		RadioMod.packetNetwork.sendToAll(new ClientUpdateComputerPacket(tileEntity.getPos(), BlockRadio.RUNSTATE_OFF));
            	}
        	}
